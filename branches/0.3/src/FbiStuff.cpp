...
// 20080108 WARNING
// 20080801 removed the loader functions from this file, as init_rd was not fimfim__init : did I break something ?
...
// 20080108 WARNING
//static void fimfim__init init_wr(void)
...
static long find_byte_stream(FILE *fp, const char *byte_stream, size_t base_offset)
{
	...
	while((rb=fread(buf+boff,1,bl-boff,fp))>0)
	{
		if(memmem(buf,rb+boff,byte_stream,sl))
			return goff+(((char*)memmem(buf,rb+boff,byte_stream,sl))-buf);
		if(rb+boff<bl)
			goto err;
		boff=sl-1;
