...
#define FIM_FLT_HELP_DISPLAY FIM_FLT_DISPLAY " ['reinit' " FIM_CNS_EX_STRING " | 'resize' " FIM_CNS_EX_W " " FIM_CNS_EX_H " | ['menuadd'|'menudel'] " FIM_CNS_EX_MENUSPEC_STRING "]: display the current file contents or change display settings." FIM_CNS_CMDSEP "If 'reinit' switch is supplied, the " FIM_CNS_EX_STRING " specifier is used to reinitialize (e.g.: change resolution, window system options) the display device." FIM_CNS_CMDSEP "See documentation for the --" FIM_OSW_OUTPUT_DEVICE " command line switch for allowed values of " FIM_CNS_EX_STRING "." FIM_CNS_CMDSEP "If 'resize' and no argument, ask the window manager to resize the window like the image." FIM_CNS_CMDSEP "If 'resize' and two arguments, these are used to reset width and height of the current window." FIM_CNS_CMDSEP "In the 'menuadd' case, use " FIM_CNS_EX_MENUSPEC_STRING	" as GTK window menu specification. This specification string argument can be repeated. Each argument has form M/S. M is a slash-separated submenu location, with '_' (underscore) before a GTK accelerator character (e.g. \"File\", \"View/Scale\", \"_View/Scale\"). S can specify a simple entry, a toggle entry, or radio buttons. If a simple entry, it consists of one to three sections separated by \"  \" (two spaces). The first section is the name of the menu entry (a label), the second is a command specification, the third is an accelerator character. A toggle button specification takes the form 'toggle__'" FIM_CNS_EX_ID_STRING "'__'" FIM_CNS_EX_VALUE "'__'" FIM_CNS_EX_VALUE " and starts the toggle button with the second " FIM_CNS_EX_VALUE " (e.g. \"menu/label  toggle___v__1__2  \"). A radio buttons specification has as many sections as radio buttons, separated by double spaces, and consisting each by a string, two spaces, an assignment " FIM_CNS_EX_ID_STRING "=" FIM_CNS_EX_VALUE " (notice no quotes are necessary around " FIM_CNS_EX_VALUE "), two spaces, and an optional accelerator character (e.g. \"m/first:  v=1  a  second:  v=2  b  third:  v=3  c\")." FIM_CNS_CMDSEP "Option 'menudel' is not implemented yet." FIM_CNS_CMDSEP "Note: 'menuadd' and 'menudel' are only valid for the " FIM_MAN_fB(FIM_DDN_INN_GTK) " mode." FIM_CNS_CMDSEP
