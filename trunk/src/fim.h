...
#define FIM_VID_WANT_MOUSE_CTRL		"_want_wm_mouse_ctrl"	/* "[in,g:] if at least 9 chars long, enable mouse click/movement behaviour when in SDL mode; the 9 chars correspond to a 3x3 screen clickable grid and the equivalent command keys; clicking middle or right button toggle on-screen usage info." */
...
#define FIM_VID_CONSOLE_KEY			"_console_key"		/* "[in,g:] the key bound (an integer variable) to spawn the command line; overrides any other binding." */
...
#define FIM_CMD_HELP_SYSTEM	FIM_FLT_SYSTEM " " FIM_CNS_EX_SYSC_STRING ": get the output of executing the " FIM_CNS_EX_SYSC_STRING " system command. Uses the popen() system call. Usually popen invokes \"" FIM_CNS_SHELL " -c " FIM_CNS_EX_SYSC_STRING "\". This might not handle a multi-word command; in that case you have to put it into a script. See 'man popen' for more."
...
#define FIM_FLT_HELP_DISPLAY FIM_FLT_DISPLAY " ['reinit' " FIM_CNS_EX_STRING " | 'resize' " FIM_CNS_EX_W " " FIM_CNS_EX_H " | ['menuadd'|'menudel'] " FIM_CNS_EX_MENUSPEC_STRING "]: display the current file contents or change display settings." FIM_CNS_CMDSEP "If 'reinit' switch is supplied, the " FIM_CNS_EX_STRING " specifier is used to reinitialize (e.g.: change resolution, window system options) the display device." FIM_CNS_CMDSEP "See documentation for the --" FIM_OSW_OUTPUT_DEVICE " command line switch for allowed values of " FIM_CNS_EX_STRING "." FIM_CNS_CMDSEP "If 'resize' and no argument, ask the window manager to resize the window like the image." FIM_CNS_CMDSEP "If 'resize' and two arguments, these are used to reset width and height of the current window." FIM_CNS_CMDSEP "In the 'menuadd' case, use " FIM_CNS_EX_MENUSPEC_STRING	" as GTK window menu specification. This specification string argument can be repeated. Each argument has form M/S. M is a slash-separated submenu location, with optional '_' (underscore) before a GTK accelerator character (e.g. \"File\", \"View/Scale\", \"_View/Scale\"). S can specify a simple entry, a toggle entry, or radio buttons. If a simple entry, it consists of one to three sections separated by \"  \" (two spaces). The first section is the name of the menu entry (a label), the second is a command specification, the third is a shortcut character. The following labels are special and if followed by '/', they create custom menus: FimMenuLimit FimMenuCommands FimMenuAliases FimMenuKeyBindings FimMenuVariables FimMenuCommandsHelp FimMenuAliasesHelp FimMenuKeyBindingsHelp FimMenuVariablesHelp. A toggle button specification takes the form 'toggle__'" FIM_CNS_EX_ID_STRING "'__'" FIM_CNS_EX_VALUE "'__'" FIM_CNS_EX_VALUE " and starts the toggle button with the second " FIM_CNS_EX_VALUE " (which has to be different). E.g. \"menu/label  toggle___v__1__2  \" is ok,  \"menu/label  toggle___v__1__1  \" is not. A radio buttons specification has as many sections as radio buttons, separated by double spaces, and consisting each by a string, two spaces, an assignment " FIM_CNS_EX_ID_STRING "=" FIM_CNS_EX_VALUE " (notice no quotes are necessary around " FIM_CNS_EX_VALUE ", and repetitions are illegal), two spaces, and an optional accelerator character (e.g. \"m/first:  v=1  a  second:  v=2  b  third:  v=3  c\" is ok, but \"m/first:  v=1  a  second:  v=2  b  third:  v=2  c\" is not)." FIM_CNS_CMDSEP "Option 'menudel' is not implemented yet." FIM_CNS_CMDSEP "Note: 'menuadd' and 'menudel' are only valid for the " FIM_MAN_fB(FIM_DDN_INN_GTK) " mode and are experimental (e.g. invalid menu specifications may lead to a crash)." FIM_CNS_CMDSEP
