...
#define FIM_VID_WANT_CAPTION_STATUS		"_want_caption_status"	/* "[internal,experimental] this works only if supported by the display device (currently only sdl). if set to a number that is not 0, will show the status (or command) line in the window manager caption; if set to a non-empty string, will interpret that string file info format string (see _info_fmt_str); if empty, will show the program version." */
