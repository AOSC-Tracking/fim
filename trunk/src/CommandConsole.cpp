...
ret:		return key;
	...
	,displaydevice_(NULL)			/* the display device could be NULL ! (FIXME) */
	{
		...
		addCommand(new Command(fim::string(FIM_FLT_LIMIT),fim::string(FIM_FLT_LIMIT " [{expression} |{variable} {value}] : A browsable file list filtering function (like limiting in the \'mutt\' program). Uses information loaded via --" FIM_OSW_LOAD_IMG_DSC_FILE ". If one argument only is provided, restrict to files whose description string matches {expression}. If two arguments are provided, restrict to files having property {variable} set to {value}. Invoked with no arguments, the original browsable files list is restored." ),&browser_,&Browser::fcmd_limit));
