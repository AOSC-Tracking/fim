...
ret:		return key;
	...
	,oldcwd_(fim_getcwd())
	{
		addCommand(new Command(FIM_FLT_ALIAS,fim::string(FIM_FLT_ALIAS " [" FIM_CNS_EX_ID_STRING " [" FIM_CNS_EX_CMDS_STRING " [" FIM_CNS_EX_DSC_STRING "]]]: without arguments, lists the current aliases. With one, shows an identifier's assigned command. With two, assigns to an identifier a user defined command or sequence of commands. With three, also assigns a help string."),this,&CommandConsole::fcmd_foo));
		addCommand(new Command(FIM_FLT_ALIGN,fim::string(FIM_CMD_HELP_ALIGN),&browser_,&Browser::fcmd_align));
#ifdef FIM_AUTOCMDS
		addCommand(new Command(FIM_FLT_AUTOCMD,fim::string(FIM_FLT_AUTOCMD " " FIM_CNS_EX_EVT_STRING " " FIM_CNS_EX_PAT_STRING " " FIM_CNS_EX_CMDS_STRING ": manipulate autocommands (inspired from Vim autocmd's). Without arguments, list autocommands. With arguments, specifies for which type of event and which current file open, which commands to execute. See the default built-in configuration files for examples."),this,&CommandConsole::fcmd_autocmd));
		addCommand(new Command(FIM_FLT_AUTOCMD_DEL,fim::string(FIM_FLT_AUTOCMD_DEL ": specify autocommands to delete them. Usage: " FIM_FLT_AUTOCMD_DEL " " FIM_CNS_EX_EVT_STRING " " FIM_CNS_EX_PAT_STRING " " FIM_CNS_EX_CMDS_STRING),this,&CommandConsole::fcmd_autocmd_del));	/* this syntax is incompatible with vim ('autocmd!')*/
#endif /* FIM_AUTOCMDS */
		addCommand(new Command(FIM_FLT_BASENAME,fim::string(FIM_FLT_BASENAME" {filename}: returns the basename of {filename} in the " FIM_VID_LAST_SYSTEM_OUTPUT " variable."),this,&CommandConsole::fcmd_basename));
		addCommand(new Command(FIM_FLT_BIND,fim::string(FIM_FLT_BIND" [" FIM_CNS_EX_KSY_STRING " [" FIM_CNS_EX_CMDS_STRING "]]: bind a keyboard symbol/shortcut " FIM_CNS_EX_KSY_STRING " to " FIM_CNS_EX_CMDS_STRING "" FIM_CNS_RAW_KEYS_MESG " Key binding is dynamical, so you can rebind keys even during program's execution. You can get a list of valid symbols (keysyms) by invoking " FIM_FLT_DUMP_KEY_CODES " or in the man page."),this,&CommandConsole::fcmd_bind));
		addCommand(new Command(FIM_FLT_CD,fim::string(FIM_CMD_HELP_CD  ),this,&CommandConsole::fcmd_cd));
#ifndef FIM_WANT_NO_OUTPUT_CONSOLE
		addCommand(new Command(FIM_FLT_CLEAR,fim::string(FIM_FLT_CLEAR ": clear the virtual console"),this,&CommandConsole::fcmd_clear));
#endif /* FIM_WANT_NO_OUTPUT_CONSOLE */
		addCommand(new Command(FIM_FLT_COMMANDS,fim::string(FIM_FLT_COMMANDS ": display the existing commands"),this,&CommandConsole::fcmd_commands_list));

		addCommand(new Command(FIM_FLT_COLOR,
		fim::string(
		...
		addCommand(new Command(FIM_FLT_DESC,fim::string(FIM_FLT_HELP_DESC),this,&CommandConsole::fcmd_desc));
		addCommand(new Command(FIM_FLT_DISPLAY,fim::string(FIM_FLT_HELP_DISPLAY),&browser_,&Browser::fcmd_display));
		addCommand(new Command(FIM_FLT_DUMP_KEY_CODES,fim::string(FIM_FLT_DUMP_KEY_CODES ": dump the active key codes (unescaped, for inspection purposes)"),this,&CommandConsole::fcmd_dump_key_codes));
		addCommand(new Command(FIM_FLT_ECHO,fim::string(FIM_FLT_ECHO " " FIM_CNS_EX_ARGS_STRING ": print the " FIM_CNS_EX_ARGS_STRING " on console"),this,&CommandConsole::fcmd_echo));
		addCommand(new Command(FIM_FLT_ELSE,fim::string(FIM_CNS_IFELSE ": see " FIM_FLT_ELSE),this,&CommandConsole::fcmd_foo));
#ifdef FIM_RECORDING
		addCommand(new Command(FIM_FLT_EVAL,fim::string(FIM_CMD_HELP_EVAL),this,&CommandConsole::fcmd_eval));
#endif /* FIM_RECORDING */
#ifndef FIM_WANT_NOSCRIPTING
		addCommand(new Command(FIM_FLT_EXEC,fim::string(FIM_FLT_EXEC " " FIM_CNS_EX_FNS_STRING ": execute script " FIM_CNS_EX_FNS_STRING ""),this,&CommandConsole::fcmd_executeFile));
#endif /* FIM_WANT_NOSCRIPTING */
#if FIM_EXPERIMENTAL_FONT_CMD
		addCommand(new Command(FIM_FLT_FONT,fim::string(
...
#endif /* FIM_EXPERIMENTAL_FONT_CMD */
		addCommand(new Command(FIM_FLT_GETENV,fim::string(FIM_FLT_GETENV " " FIM_CNS_EX_ID_STRING ": display the value of the " FIM_CNS_EX_ID_STRING " environment variable"),this,&CommandConsole::fcmd_do_getenv));
		addCommand(new Command(FIM_FLT_GOTO,fim::string(FIM_CMD_HELP_GOTO),&browser_,&Browser::fcmd_goto_image));
		addCommand(new Command(FIM_FLT_HELP,fim::string(FIM_CMD_HELP_HELP),this,&CommandConsole::fcmd_help));
		addCommand(new Command(FIM_FLT_IF,fim::string(FIM_CNS_IFELSE ": see " FIM_FLT_ELSE),this,&CommandConsole::fcmd_foo));
		addCommand(new Command(FIM_FLT_INFO,fim::string(FIM_FLT_INFO ": display information about the current file" ),&browser_,&Browser::fcmd_info));
		...
		addCommand(new Command(FIM_FLT_LIST,fim::string(FIM_CMD_HELP_LIST),&browser_,&Browser::fcmd_list));
		addCommand(new Command(FIM_FLT_LOAD,fim::string(FIM_FLT_LOAD ": load the image, if not yet loaded (see also " FIM_FLT_RELOAD ")"),&browser_,&Browser::fcmd_load));
		addCommand(new Command(FIM_FLT_PAN,fim::string(FIM_FLT_PAN " {'down'|'up'|'left'|'right'|'ne'|'nw'|'se'|'sw'}[+-] [{steps}['%']]: pan the image {steps} pixels in the desired direction;" " if the '%' specifier is present, {steps} will be treated as a percentage of current screen dimensions;" " if {steps} is not specified, the \"" FIM_VID_STEPS "\" variable will be used;" " if present, the \"" FIM_VID_HSTEPS "\" variable will be considered for horizontal panning;" "a + or - sign at the end of the first argument will make jump to next or prev if border is reached;" " if present, the \"" FIM_VID_VSTEPS "\" variable will be considered for vertical panning;" " the variables may be terminated by the \'%\' specifier" " "),&browser_,&Browser::pan));
		addCommand(new Command(FIM_FLT_POPEN,fim::string(FIM_FLT_POPEN " " FIM_CNS_EX_SYSC_STRING ": pipe a command, invoking popen(): spawns a shell, invoking " FIM_CNS_EX_SYSC_STRING " and executing as fim commands the output of " FIM_CNS_EX_SYSC_STRING),this,&CommandConsole::fcmd_sys_popen));
#ifdef FIM_PIPE_IMAGE_READ
		addCommand(new Command(FIM_FLT_PREAD,fim::string(FIM_FLT_PREAD " " FIM_CNS_EX_ARGS_STRING ": execute " FIM_CNS_EX_ARGS_STRING " as a shell command and read the output as an image file (using " FIM_FLT_POPEN ")"),this,&CommandConsole::fcmd_pread));
#endif /* FIM_PIPE_IMAGE_READ */
		addCommand(new Command(FIM_FLT_PREFETCH,fim::string(FIM_FLT_PREFETCH ": prefetch (read into the cache) the two nearby image files (next and previous), for a faster subsequent opening. See also the " FIM_VID_WANT_PREFETCH 			 " variable."),&browser_,&Browser::fcmd_prefetch));
		addCommand(new Command(FIM_FLT_PWD,fim::string(FIM_CMD_HELP_PWD),this,&CommandConsole::fcmd_pwd));
		addCommand(new Command(FIM_FLT_QUIT,fim::string(FIM_FLT_QUIT " [{number}]: terminate the program; if {number} is specified, use it as the program return status; autocommand \"" FIM_ACM_POSTINTERACTIVECOMMAND "\" does not trigger after this command"),this,&CommandConsole::fcmd_quit));
#ifdef FIM_RECORDING
		addCommand(new Command(FIM_FLT_RECORDING,fim::string(
...
#endif /* FIM_RECORDING */
		addCommand(new Command(FIM_FLT_REDISPLAY,fim::string(FIM_FLT_REDISPLAY ": re-display the current file contents"),&browser_,&Browser::fcmd_redisplay));
		addCommand(new Command(FIM_FLT_RELOAD,fim::string(FIM_FLT_RELOAD" [{arg}]: load the image into memory; if {arg} is present, will force reloading, bypassing the cache (see also " FIM_FLT_LOAD ")"),&browser_,&Browser::fcmd_reload));
		addCommand(new Command(FIM_FLT_ROTATE,fim::string(FIM_FLT_ROTATE " " FIM_CNS_EX_NUM_STRING ": rotate the image the specified amount of degrees [undocumented]" ),&browser_,&Browser::fcmd_rotate));
		addCommand(new Command(FIM_FLT_SCALE,fim::string(FIM_CMD_HELP_SCALE),&browser_,&Browser::fcmd_scale));
		addCommand(new Command(FIM_FLT_SCROLLDOWN,fim::string(FIM_FLT_SCROLLDOWN ": scroll down the image, going next if at bottom" ),&browser_,&Browser::fcmd_scrolldown));
		addCommand(new Command(FIM_FLT_SCROLLFORWARD,fim::string(FIM_FLT_SCROLLFORWARD ": scroll the image as it were reading it" ),&browser_,&Browser::fcmd_scrollforward));
		addCommand(new Command(FIM_FLT_SET,fim::string(FIM_CMD_HELP_SET),this,&CommandConsole::fcmd_set));
		addCommand(new Command(FIM_FLT_SET_CONSOLE_MODE,fim::string(FIM_FLT_SET_CONSOLE_MODE ": set console mode"),this,&CommandConsole::fcmd_set_in_console));
		addCommand(new Command(FIM_FLT_SET_INTERACTIVE_MODE,fim::string(FIM_FLT_SET_INTERACTIVE_MODE ": set interactive mode"),this,&CommandConsole::fcmd_set_interactive_mode));
		addCommand(new Command(FIM_FLT_SLEEP,fim::string(FIM_FLT_SLEEP " [" FIM_CNS_EX_NUM_STRING "=1]: sleep for the specified (default 1) number of seconds"),this,&CommandConsole::fcmd_foo));
		addCommand(new Command(FIM_FLT_STATUS,fim::string(FIM_FLT_STATUS ": set the status line to the collation of the given arguments"),this,&CommandConsole::fcmd_status));
		addCommand(new Command(FIM_FLT_STDOUT,fim::string(FIM_FLT_STDOUT " " FIM_CNS_EX_ARGS_STRING ": writes to stdout its arguments " FIM_CNS_EX_ARGS_STRING ""),this,&CommandConsole::fcmd__stdout));
#ifndef FIM_NO_SYSTEM
		addCommand(new Command(FIM_FLT_SYSTEM,fim::string(FIM_CMD_HELP_SYSTEM),this,&CommandConsole::fcmd_system));
#endif /* FIM_NO_SYSTEM */
		addCommand(new Command(FIM_FLT_VARIABLES,fim::string(FIM_FLT_VARIABLES ": display the existing variables"),this,&CommandConsole::fcmd_variables_list));
		addCommand(new Command(FIM_FLT_UNALIAS,fim::string(FIM_FLT_UNALIAS " " FIM_CNS_EX_ID_STRING " | '-a': delete the alias " FIM_CNS_EX_ID_STRING " or all aliases (use '-a', not -a)"),this,&CommandConsole::fcmd_unalias));
		addCommand(new Command(FIM_FLT_UNBIND,fim::string(FIM_FLT_UNBIND " " FIM_CNS_EX_KSY_STRING ": unbind the action associated to a specified " FIM_CNS_EX_KSY_STRING FIM_CNS_RAW_KEYS_MESG),this,&CommandConsole::fcmd_unbind));
		addCommand(new Command(FIM_FLT_WHILE,fim::string("while(expression){action;}:  A conditional cycle construct. May be interrupted by hitting the " FIM_KBD_ESC " or the " FIM_KBD_COLON " key."),this,&CommandConsole::fcmd_foo));/* may introduce a special "help grammar" command */
#ifdef FIM_WINDOWS
		/* this is a stub for the manual generation (actually, the FimWindow object gets built later) */
		addCommand(new Command(FIM_FLT_WINDOW,fim::string(FIM_CMD_HELP_WINDOW),this,&CommandConsole::fcmd_foo));
#endif /* FIM_WINDOWS */
