...
fim_var_help_db[FIM_VID_DISPLAY_STATUS_FMT]= "[internal,in,experimental] format of display status string; if unset: full pathname; otherwise a custom format string (see _info_fmt_str). " ;
...
fim_var_help_db[FIM_VID_WANT_CAPTION_STATUS]= "[internal,experimental] this works only if supported by the display device (currently only sdl). if set to a number that is not 0, will show the status (or command) line in the window manager caption; if set to a non-empty string, will interpret that string file info format string (see _info_fmt_str); if empty, will show the program version." ;
