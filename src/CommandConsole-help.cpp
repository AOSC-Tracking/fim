...
#include "help-acm.cpp"
		...
		if(args.size()==1 && args[0]=="man")
		{
			...
			string("One may exit from command line mode by pressing the "FIM_KBD_ENTER" key on an empty line (a non empty command line would be submitted for execution), or the "FIM_KBD_ESC" key (clearing the current command line).\n")+
