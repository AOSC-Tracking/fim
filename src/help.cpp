...
fim_var_help_db[FIM_VID_INFO_FMT_STR]= "[internal,in] custom (status bar) file info format string; may contain ordinary text and special sequences; these are: %p for scale, in percentage; %w for width; %h for height; %i for image index in list; %l for image list length; %L for flip/mirror information; % for page information; %F for file size; %M for memory image size; %C for memory used by cache; %T for total memory used; %% for an ordinary %. This feature has to be enabled at configure time." ;
