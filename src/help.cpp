...
fim_var_help_db[FIM_VID_WANT_EXIF_ORIENTATION]= "[internal,in] if 1, will reorient images using information from EXIF metadata (and stored in in " FIM_VID_EXIF_ORIENTATION ", " FIM_VID_EXIF_MIRRORED ", " FIM_VID_EXIF_FLIPPED" )." ;
fim_var_help_db[FIM_VID_EXIF_ORIENTATION]= "[internal,out] orientation information in the same format of " FIM_VID_ORIENTATION ", read from the EXIF tags of a given image (i:)." ;
fim_var_help_db[FIM_VID_EXIF_MIRRORED]= "[internal,out] mirroring information, read from the EXIF tags of a given image (i:)." ;
fim_var_help_db[FIM_VID_EXIF_FLIPPED]= "[internal,out] flipping information, read from the EXIF tags of a given image (i:)." ;
...
fim_var_help_db[FIM_VID_INFO_FMT_STR]= "[internal,in] custom (status bar) info format string, displayed in the lower right corner; may contain ordinary text and special 'expando' sequences. These are: %p for current scale, in percentage; %w for width; %h for height; %i for image index in list; %l for image list length; %L for flip/mirror/orientation information; %P for page information; %F for file size; %M for screen image memory size; %m for memory used by mipmap; %C for memory used by cache; %T for total memory used (approximation); %n for the current file path name; %N for the current file path name basename; ; %c for centering information; %v for the fim program/version identifier string; %% for an ordinary %. A sequence like %?VAR?EXP? expands to EXP if i:VAR is set; EXP will be copied verbatim except for contained sequences of the form %:VAR:, which will be expanded to the value of variable i:VAR; this is meant to be used like in e.g. 'EXIF_DateTimeOriginal?[%:EXIF_DateTimeOriginal:]?', where an EXIF-set variable (make sure you have libexif for this) will be used only if present." ;
...
fim_var_help_db[FIM_VID_PAGES]= "[internal,out,experimental] the current number of pages of an image (i:) " ;
...
fim_var_help_db[FIM_VID_SAVE_FIM_HISTORY]= "[internal,in] if 1 on exit, will save the " FIM_CNS_HIST_COMPLETE_FILENAME " file on exit" ;
fim_var_help_db[FIM_VID_LOAD_FIM_HISTORY]= "[internal,in] if 1 on startup, will load the " FIM_CNS_HIST_COMPLETE_FILENAME " file on startup" ;
...
fim_var_help_db[FIM_VID_LAST_FILE_LOADER]= "[internal,out] after each image load, " FIM_VID_LAST_FILE_LOADER " will be set to the last file loader used" ;
...
fim_var_help_db[FIM_VID_OPEN_OFFSET]= "[internal,in,optional] offset (specified in bytes) used when opening a file; [internal,out] i:" FIM_VID_OPEN_OFFSET " will be assigned to images opened at a nonzero offset " ;
fim_var_help_db[FIM_VID_OPEN_OFFSET_RETRY]= "[internal,in,optional] number of adjacent bytes to probe in opening the file " ;
...
fim_var_help_db[FIM_VID_PRELOAD_CHECKS]= "[internal,in,experimental] if 1 (default), will check with stat() existence of input files before push'ing them (set this to 0 to speed up loading very long file lists; in these cases a trailing slash (/) will have to be used to tell fim a pathname is a directory). This only works after initialization (thus, after command line files have been push'ed); use --no-stat-push if you wish to set this to 0 at command line files specification " ;
