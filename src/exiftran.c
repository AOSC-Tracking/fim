...
int main(int argc, char *argv[])
{
    ...
    for (;;) {
	c = getopt(argc, argv, "hbpid912fFtTac:o:n:");
	if (c == -1)
	    break;
	switch (c) {
	case '9':
    ...
    if (JXFORM_NONE == transform && NULL == comment) {
	fprintf(stderr,
    ...
    if (outfile) {
	return jpeg_transform_files(argv[optind], outfile, transform, comment, flags);
    } else {
	rc = 0;
	for (i = optind; i < argc; i++) {
	    fprintf(stderr,"processing %s\n",argv[i]);
	    if (0 != jpeg_transform_inplace(argv[i], transform, comment, flags))
		rc = 1;
	}
	return rc;
    }
