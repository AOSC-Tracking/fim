...
//	void  AADevice::display(char* rgb, int rows, int ldcols, int cols, int flags)
	...
	)
	{
		...
		if( oroff>orows ) return -7-9*100;
		if( ocoff>ocols ) return -8-10*100;

		if( icskip<icols ) return -6-5*100;
		if( ocskip<ocols ) return -11-10*100;
		/* temporary sanitizers */
...
/*		cout << oroff << " " << ocoff << " " << orows << " " << ocols << " " << ocskip << "\n";
		...
		if(r=matrix_copy_rgb_to_gray(
				...
				orows,ocols,// rows and columns to draw in output buffer
