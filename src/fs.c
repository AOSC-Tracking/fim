...
void *memset_combine(void *s, int c, size_t n)
{
	/*
	 * this function is ridicously slow
	 */
	if(!s)return s;
//	char *ss=s;char *sc=s+n;
//	for(;ss<sc;++ss)*ss=((*ss)<<3)|(c>>5);
	register int mask=0x0F0F0F0F;
	mask = 0x4F4F4F4F;
	register int imask=0^mask;
	c&=imask;
	register int *ip=(int*)s;
	register int *fip=ip+(n>>2);
	for(;ip<fip;++ip) *ip=( *ip & mask ) | c;
	//from 0 to 3 bytes are missing, but this is fun for now :)
	return s;
}
