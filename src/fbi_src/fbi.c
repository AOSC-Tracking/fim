...
static void status(const unsigned char *desc, const char *info)
{
    int chars, ilen;
    char *str;
...
/*    for (tag = 0; tag < ARRAY_SIZE(tags); tag++)
	...
	free(linebuffer[i]);
#else
    status("compiled without EXIF support, sorry",NULL);
#endif
}
...
static void scale_fix_top_left(float old, float fnew, struct ida_image *img)
{
    ...
    width  = img->i.width  * fnew;
    height = img->i.height * fnew;
...
static int read_comment(char *filename, char *dest, int len)
{
    int fh,rc,i;
...
/*
static void write_comment(char *filename, char *dest, int len)
{
    int fh;

    if (-1 == (fh = open(filename, O_WRONLY | O_CREAT | O_TRUNC, 0666))) {
	show_error("writing comment failed");
	return;
    }
    write(fh,dest,len);
    write(fh,"\n",1);
...
static char *file_desc(char *filename)
{
...
/*char *make_desc(struct ida_image_info *img, char *filename)*/
{
	...
	desc = file_desc(filename);
	len = read_comment(desc, linebuffer, sizeof(linebuffer)-1);
	if (-1 != len)
...
static void edit_desc(char *filename)
{
    ...
    if (0 != rc)
	return;
    write_comment(desc,linebuffer,strlen(linebuffer));
}*/
