...
static void do_thumbnail(ExifData *ed, JXFORM_CODE transform)
{
    ...
    /* transform image */
    do_transform(&th.src,&th.dst,transform,NULL,JFLAG_TRANSFORM_IMAGE);
...
static void do_exif(struct jpeg_decompress_struct *src,
		    JXFORM_CODE *transform, unsigned int flags)
{
		      ...
		      unsigned int flags)
{
    ...
    /* transform image */
    do_transform(&src,&dst,transform,comment,flags);
			 ...
			 unsigned int flags)
{
    ...
    /* go! */
    rc = jpeg_transform_fp(in,out,transform,comment,flags);
    fclose(in);
    fclose(out);
			   ...
			   unsigned int flags)
{
    ...
    /* transform */
    if (0 != jpeg_transform_fp(in,out,transform,comment,flags))
	goto oops;
