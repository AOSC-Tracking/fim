...
#define interactive_mode_ 0
	...
	std::map<std::string,std::string> aliases_; // TODO: FIXME: temporary
...
void do_rebuild_help_aliases_menu(GtkWidget *aliaMi, const bool help_or_cmd)
{
	...
	for (const auto & ali : aliases_)
	{
		GtkWidget * const aliMi = gtk_menu_item_new_with_label(ali.first.c_str());
		gtk_widget_set_tooltip_text(aliMi, ali.second.c_str() );
		if ( aliases_.find(ali.first) != aliases_.end() )
		{
			if ( help_or_cmd )
...
void do_rebuild_help_commands_menu(GtkWidget *cmdsMi, const bool help_or_press)
{
	...
	for (const auto & cf : cmd_funcs_) // cmd_to_seq_
	{
		const auto & cmd = cf.first;
		const auto & csi = cmd_to_seq_.find(cmd);
		GtkWidget * const cmdMi = gtk_menu_item_new_with_label(cmd.c_str());
		gtk_widget_set_tooltip_text(cmdMi, (cmd + ": " + do_get_item_help(cf.first.c_str())).c_str() ); // help msg
...
repeat:
	...
	if ( ( e = strstr(b, st) ) || ( e = b + strlen(b) ) )
	{
		...
		if (cmd.size())
		{
			...
			if ( cmd_funcs_.find(cmd) != cmd_funcs_.end() )
			{
				g_signal_connect(G_OBJECT(menu_items_[add]), "activate", G_CALLBACK(cmd_funcs_["STUB"]), NULL);
...
static void do_init_cmd_funcs(void)
{
	...
	cmd_funcs_["magnify"] = [](){ };
	cmd_funcs_["next"] = [](){ std::cout << "jumping to next\n"; };
	cmd_funcs_["prev"] = [](){ std::cout << "jumping to prev\n"; };
	cmd_funcs_["shrink"] = [](){ };
	cmd_funcs_["scale_set_auto"] = [](){ };
	...
	cmd_funcs_["readline_on"] = [](){  };
	cmd_funcs_["fwsrchln_on"] = [](){  };
	cmd_funcs_["bwsrchln_on"] = [](){  };
	cmd_funcs_["unmapped_cmd"] = [](){ std::cout << "unmapped cmd\n"; };
	cmd_funcs_["add_new_alias_and_variable"] = [](){ 
		...
		aliases_["new_alias"]="new_alias_value";
...
void do_rebuild_help_menus(void)
{
	...
	cmd_funcs_["STUB"] = [](){ std::cout << "STUB\n"; }; // TODO: FIXME: replace this stub
