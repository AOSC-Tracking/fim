...
#define FIM_CMD_HELP_LIST	FIM_FLT_LIST ": display the files list; " FIM_CNS_CMDSEP FIM_FLT_LIST " 'random_shuffle': randomly shuffle the file list;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'reverse': reverse the file list;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'clear': clear the file list;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'sort': sort the file list;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'sort_basename': sort the file list according to base name;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'sort_fsize': sort the file list according to file size;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'sort_mtime': sort the file list according to modification date; " FIM_CNS_CMDSEP FIM_FLT_LIST " 'pop' : pop the last file from the files list;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'remove' [" FIM_CNS_EX_FNS_STRING "]: remove the current file, or the " FIM_CNS_EX_FNS_STRING ", if specified;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'push' " FIM_CNS_EX_FNS_STRING " : push " FIM_CNS_EX_FNS_STRING " to the back of the files list; " FIM_CNS_CMDSEP FIM_FLT_LIST " 'filesnum': display the number of files in the files list;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'mark' [{args}] : mark image file names for stdout printing at exit, with {args} mark the ones matching according to the rules of the '" FIM_FLT_LIMIT "' command, otherwise the current file;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'unmark' [{args}] : unmark marked image file names, with {args} unmark the ones matching according to the rules of the '" FIM_FLT_LIMIT "' command, otherwise the current file;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'marked': show which files have been marked so far;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'dumpmarked': dump to stdout the marked files (you will want usually to 'unmarkall' afterwards);" FIM_CNS_CMDSEP FIM_FLT_LIST " 'unmarkall': unmark all the marked files;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'pushdir'; {dirname}: will push all the files in {dirname}, when matching the regular expression in variable " FIM_VID_PUSHDIR_RE " or, if empty, from constant regular expression " FIM_CNS_PUSHDIR_RE ";" FIM_CNS_CMDSEP FIM_FLT_LIST " 'pushdirr' {dirname}: like pushdir, but will also push encountered directory entries recursively;" FIM_CNS_CMDSEP FIM_FLT_LIST " 'swap': will move the current image filename to the first in the list (you'll have to invoke " FIM_FLT_RELOAD " to see the effect)." FIM_CNS_CMDSEP "Of the above commands, several will be temporarily non available for the duration of a background load (enabled by --" FIM_OSW_BGREC "), which will last until " FIM_VID_LOADING_IN_BACKGROUND " is 0."
...
#define FIM_CMD_HELP_PAN    FIM_FLT_PAN " {'down'|'up'|'left'|'right'|'ne'|'nw'|'se'|'sw'}[+-] [{steps}['%']]: pan the image {steps} pixels in the desired direction;" FIM_CNS_CMDSEP " if the '%' specifier is present, {steps} will be treated as a percentage of current screen dimensions;" FIM_CNS_CMDSEP " if {steps} is not specified, the \"" FIM_VID_STEPS "\" variable will be used;" FIM_CNS_CMDSEP " if present, the \"" FIM_VID_HSTEPS "\" variable will be considered for horizontal panning;" "a + or - sign at the end of the first argument will make jump to next or prev if border is reached;" FIM_CNS_CMDSEP " if present, the \"" FIM_VID_VSTEPS "\" variable will be considered for vertical panning." FIM_CNS_CMDSEP " The variables may be terminated by the \'%\' specifier."
...
#define FIM_CMD_HELP_WINDOW FIM_FLT_WINDOW " " FIM_CNS_EX_ARGS_STRING ": this command is disabled. It might be re-enabled in a future version."
